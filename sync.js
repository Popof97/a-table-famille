...
async function pull(){try{const response=await fetch(`${SUPABASE_URL}/rest/v1/family_state?id=eq.${FAMILY_ID}&select=data`,{headers});if(!response.ok)throw new Error();const rows=await response.json();if(!rows.length){syncStatus.textContent='Synchronisation prete';return;}const remote=stableJson(rows[0].data),local=stableJson(snapshot());if(remote!==lastRemote&&remote!==local){syncing=true;applySnapshot(rows[0].data);lastRemote=remote;syncStatus.textContent='Mise a jour recue';setTimeout(()=>location.reload(),400);return;}lastRemote=remote;syncStatus.textContent='Synchronise';}catch{syncStatus.textContent='Synchronisation a activer';}}
let timer;
